  ...
  return (
    <div className="fixed top-6 left-6 z-40">
      <div
        ...
        style={{ background: 'rgba(255, 255, 255, 0.92)', backdropFilter: 'blur(12px)', minWidth: '220px', padding: '18px 20px' }}
      >
        <div className="flex items-center justify-between" style={{ marginBottom: '14px' }}>
          <span className="text-base font-extrabold text-gray-800">
            {carMode === 'rc' ? '🎮 أجزاء الريموت' : '⭐ اكتشف الأجزاء'}
          </span>
          <span
            className="text-sm font-bold rounded-full"
            style={{ background: count === total ? '#06D6A0' : '#F3F4F6', color: count === total ? 'white' : '#6B7280', padding: '3px 12px' }}
          >
            {count} / {total}
          </span>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(5, total)}, 1fr)`, gap: '8px', marginBottom: '14px' }}>
          {partsData.map((part, index) => {
            const isDiscovered = discoveredParts.includes(part.id)
            return (
              <div
                key={part.id}
                className={`flex items-center justify-center text-base font-bold transition-all duration-500 ${isDiscovered ? 'animate-star-pop shadow-md' : ''}`}
                style={{
        ...
        <div className="rounded-full overflow-hidden" style={{ height: '10px', background: '#F3F4F6' }}>
          <div
            ...
            style={{
              ...
              background: count === total ? '#06D6A0' : carMode === 'rc'
                ? 'linear-gradient(90deg, #E91E63, #9C27B0, #3F51B5)'
