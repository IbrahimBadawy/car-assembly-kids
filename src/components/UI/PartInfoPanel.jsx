...
export default function PartInfoPanel() {
  ...
  const part = partsData.find((p) => p.id === selectedPart)
  ...
  return (
    <div className="fixed right-5 top-1/2 z-40" style={{ transform: 'translateY(-50%)' }}>
      <div
        ...
        style={{ width: '300px', background: 'white', border: `4px solid ${part.color}` }}
      >
        <button
        ...
        <div className="w-full flex items-center justify-center" style={{ height: '176px', backgroundColor: `${part.color}12`, padding: '16px' }}>
          <img
        ...
        <div style={{ padding: '20px 24px' }}>
          <h2 className="text-center font-black" style={{ fontSize: '1.5rem', color: part.color, marginBottom: '10px' }}>
            {part.icon} {part.nameAr}
          </h2>
          <div className="rounded-full mx-auto" style={{ width: '48px', height: '4px', backgroundColor: part.color, marginBottom: '12px' }} />
          <p className="text-gray-600 text-center font-semibold" style={{ fontSize: '1rem', lineHeight: '1.8', marginBottom: '12px' }}>
            {part.description}
          </p>
          ...
          {isNarrating && (
            <div className="flex items-center justify-center" style={{ gap: '8px', padding: '8px 0' }}>
              <div className="flex items-end" style={{ gap: '3px', height: '20px' }}>
                {[0, 1, 2, 3, 4].map((i) => (
                  <div key={i} className="rounded-full" style={{ width: '3px', backgroundColor: part.color, animation: `wave 0.5s ease-in-out ${i * 0.08}s infinite alternate` }} />
                ))}
              </div>
        ...
        <style>{`@keyframes wave { 0% { height: 4px; } 100% { height: 18px; } }`}</style>
      </div>
    </div>
  )
}
