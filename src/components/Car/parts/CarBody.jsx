...
export default function CarBody() {
  ...
  return (
    <group>
      {/* Main body - lower */}
      <mesh position={[0, -0.05, 0]} castShadow raycast={isXray ? () => {} : undefined}>
      ...
      {/* Cabin - upper */}
      <mesh position={[0, 0.55, 0.1]} castShadow raycast={isXray ? () => {} : undefined}>
      ...
      {/* Hood slope (front) */}
      <mesh position={[0, 0.25, -1.35]} rotation={[0.15, 0, 0]} castShadow raycast={isXray ? () => {} : undefined}>
      ...
      {/* Trunk slope (rear) */}
      <mesh position={[0, 0.2, 1.4]} rotation={[-0.1, 0, 0]} castShadow raycast={isXray ? () => {} : undefined}>
        <boxGeometry args={[1.9, 0.08, 0.7]} />
        <meshStandardMaterial color={accentColor} roughness={0.3} metalness={0.15} transparent opacity={opacity} />
      </mesh>

      {/* Front bumper */}
      <mesh position={[0, -0.25, -1.95]} castShadow raycast={isXray ? () => {} : undefined}>
      ...
      {/* Roof rack accent */}
      <mesh position={[0, 0.84, 0.1]} raycast={isXray ? () => {} : undefined}>
